...
import { Button, Card, Alert, BrandLogo } from "@/components/ui";
...
export default function OnboardingPage() {
  ...
  // Dynamically tailor calibration questions to whichever career the user chooses (e.g. Video Editor)
  useEffect(() => {
    const activeRole = customRole.trim() || selectedRole;
    if (activeRole) {
      const tailored = getOnboardingCalibrationQuestions(activeRole, 4);
      setAssessmentQuestions(tailored);
      setQuizAnswers({});
    }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between selection:bg-primary selection:text-primary-foreground">
      {/* Top Header */}
      <header className="border-b border-border bg-background/85 backdrop-blur-md px-6 py-4 flex items-center justify-between sticky top-0 z-40">
        <BrandLogo size="sm" />
      ...
      <main className="flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-8">
        <div className="w-full max-w-2xl">
          <Card variant="default" className="p-6 sm:p-8 shadow-2xl bg-card border-border">
            {/* Quick Skip to Dashboard if details already filled */}
            <div className="mb-5 p-3 rounded-xl border border-primary/20 bg-primary/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
